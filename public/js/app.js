...
(function(){
  ...
  // ── Storage helper (aman untuk preview & quota penuh) ──
  ...
  function persist(key, value){
    try {
      ...
      if (!storageNotified) {
        ...
        setTimeout(function(){
          try {
            alert('⚠️ Penyimpanan browser tidak tersedia atau sudah penuh. ' +
              'Data (termasuk gambar) hanya tersimpan sementara di memori dan akan hilang saat halaman dimuat ulang. ' +
              'Buka website langsung di browser (bukan mode preview) dan gunakan gambar berukuran wajar.');
          } catch(_) {}
        }, 60);
      }
      return false;
  ...
  function saveOrders(){ persist('czen_orders', JSON.stringify(orders)); }
  ...
  function saveUsers(){ persist('czen_users', JSON.stringify(usersDB)); }
  ...
  function saveStickers(){ persist('czen_stickers', JSON.stringify(customStickers)); }
  ...
  function saveFrames(){ persist('czen_frames', JSON.stringify(customFrames)); }

  // ── Wishlist (per-user) ──────────────────────────────
  var wishlist = (function(){
    try { return JSON.parse(localStorage.getItem('czen_wishlist') || '{}'); }
    catch(e) { return {}; }
  })();
  function saveWishlist(){ persist('czen_wishlist', JSON.stringify(wishlist)); }

  // ── Reviews / Testimonials ───────────────────────────
  var reviews = (function(){
    try { return JSON.parse(localStorage.getItem('czen_reviews') || '[]'); }
    catch(e) { return []; }
  })();
  function saveReviews(){ persist('czen_reviews', JSON.stringify(reviews)); }

  // ── Photobooth bookings ──────────────────────────────
  var bookings = (function(){
    try { return JSON.parse(localStorage.getItem('czen_bookings') || '[]'); }
    catch(e) { return []; }
  })();
  function saveBookings(){ persist('czen_bookings', JSON.stringify(bookings)); }
  ...
  window.CZEN = {
    ...
    // URL gambar utama produk (prioritas: galeri[0], lalu img)
    thumb: function(p){
      if(!p) return 'public/images/1.png';
      var s = (p.images && p.images.length) ? p.images[0] : (p.img || '1');
      s = String(s || '1');
    ...
    auth: {
      ...
      setUser: function(u){
        this._current = u;
        persist('czen_user', JSON.stringify(u));
      },
      logout: function(){
        this._current = null;
        try { localStorage.removeItem('czen_user'); }
        catch(e) {}
      },
    ...
    cart: {
      ...
      _load: function(){
        try {
          var v = localStorage.getItem('czen_cart');
          if (v !== null && v !== undefined) { this._mem = JSON.parse(v); return this._mem; }
        } catch(e) {}
        if (this._mem === null) this._mem = [];
        return this._mem;
      },
      _save: function(items){
        this._mem = items;
        persist('czen_cart', JSON.stringify(items));
      },
      add: function(id,qty,color){
        qty=qty||1; var items=this._load();
        var f=items.find(function(it){return it.id===id && (it.color||'')===(color||'');});
        if(f)f.qty+=qty; else items.push({id:id,qty:qty,color:color||null});
        this._save(items); this.updateBadge();
      },
      remove: function(id,color){
        this._save(this._load().filter(function(it){return !(it.id===id && (it.color||'')===(color||''));}));
        this.updateBadge();
      },
      setQty: function(id,qty,color){
        if(qty<1){this.remove(id,color);return;}
        var items=this._load(); var f=items.find(function(it){return it.id===id && (it.color||'')===(color||'');});
        if(f)f.qty=qty; else items.push({id:id,qty:qty,color:color||null});
        this._save(items); this.updateBadge();
      },
      items: function(){
        return this._load().map(function(it){
          ...
          if(!p) return null;
          var s=(p.images&&p.images.length)?p.images[0]:(p.img||'1');
          return {id:p.id,name:p.name,img:s,price:p.price,qty:it.qty,color:it.color||null,sub:p.price*it.qty};
        }).filter(Boolean);
      },
      ...
      reader.onload = function(e){
        ...
        img.onload = function(){
          try {
            ...
            var out = asPng ? c.toDataURL('image/png') : c.toDataURL('image/jpeg', quality);
            done(out);
          } catch(err) { done(raw); }
        };
    ...
    // ── Wishlist CRUD (user) ───────────────────────────
    _wishlist: {
      list: function(email){ return (wishlist[email] || []).slice(); },
      has: function(email, id){ return (wishlist[email] || []).indexOf(id) !== -1; },
      toggle: function(email, id){
        if (!wishlist[email]) wishlist[email] = [];
        var i = wishlist[email].indexOf(id);
        if (i === -1) wishlist[email].push(id); else wishlist[email].splice(i, 1);
        saveWishlist();
        return wishlist[email].slice();
      },
      remove: function(email, id){
        if (!wishlist[email]) return [];
        wishlist[email] = wishlist[email].filter(function(x){ return x !== id; });
        saveWishlist();
        return wishlist[email].slice();
      }
    },
    ...
    _reviews: {
      list: function(){ return reviews.slice(); },
      add: function(r){
        r.id = 'rev-' + Date.now().toString(36);
    ...
    // ── Photobooth bookings CRUD ───────────────────────
    _bookings: {
      list: function(){ return bookings.slice(); },
      add: function(b){
        b.id = b.id || 'PB-' + Date.now().toString(36).toUpperCase();
      ...
      update: function(id, data){
        ...
        if (idx === -1) return null;
        Object.keys(data).forEach(function(k){ bookings[idx][k] = data[k]; });
        saveBookings();
        return bookings[idx];
      },
      remove: function(id){
    ...
    // ── Photobooth schedule helper (08:00-18:00, 5 menit/sesi) ──
    _photobooth: {
      open: '08:00',
      close: '18:00',
      slotMin: 5,
